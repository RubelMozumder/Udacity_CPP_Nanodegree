...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle) {
  ...
  lck.unlock();
  if (TrafficlightObj.getCurrentPhase() == TrafficLightPhase::red)
    TrafficlightObj.waitForGreen();
};
...
void Intersection::simulate()  // using threads + promises/futures + exceptions
{
  ...
  threads.emplace_back(std::thread(&TrafficLight::simulate, &TrafficlightObj));
}
...
void Intersection::processVehicleQueue() {
  ...
  while (true) {
    ...
    if (!trafficLightIsGreen()) continue;
...
bool Intersection::trafficLightIsGreen() {
  if (TrafficlightObj.getCurrentPhase() == TrafficLightPhase::green)
    return true;
  else
    return false;
